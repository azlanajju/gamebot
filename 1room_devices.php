<?php
include("./config.php");
$devices = '<div class="row">'; // Start a new row container

if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['room_id'])) {
  $roomId = $_GET['room_id'];
  $sql = "SELECT * FROM Displays WHERE room_id = $roomId";
  ...
  if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $status = $row['status'];
      // Check if the device is active and apply the appropriate CSS class
      $cssClass = ($status == 'Active') ? 'active-device' : '';
      // Append each device inside the while loop
      $devices .= '<a href="1device_details.php?d_id=' . $displayId . '" class="col-md-2"><div id="display' . $displayId . '" class="grid-display ' . $cssClass . '">' . $displayName . '</div></a>';
    }
  } else {
    $devices .= '<div class="col">No devices found for the specified room ID.</div>';
  }
} else {
  $devices .= '<div class="col">Invalid request.</div>';
}
$devices .= '</div>'; // Close the row container
mysqli_close($conn);
?>
...
<head>
  ...
  <title>Devices / Devices - NiceAdmin Bootstrap Template</title>
