...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['roomName'])) {
  $roomName = $_POST['roomName'];

  $sqlInsertRoom = "INSERT INTO Rooms (room_name) VALUES ('$roomName')";
  if (mysqli_query($conn, $sqlInsertRoom)) {
    $roomId = mysqli_insert_id($conn);

    for ($i = 1; $i <= 30; $i++) {
      $dummyDisplayName = "Display $i";
      $dummyIpAddress = "192.168.0.$i";
      $dummyStatus = "Inactive";
      $sqlInsertDevice = "INSERT INTO Displays (room_id, display_name, ip_address, status) 
                                VALUES ($roomId, '$dummyDisplayName', '$dummyIpAddress', '$dummyStatus')";
      mysqli_query($conn, $sqlInsertDevice);
    }

    echo "Room created successfully with 30 devices.";
  } else {
    echo "Error creating room: " . mysqli_error($conn);
  }
} else {
