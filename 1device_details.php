...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['display_id'])) {
    ...
    $sql = "UPDATE Displays SET display_name = '$displayName', ip_address = '$ipAddress', status = '$status' WHERE display_id = $displayId";
    if (mysqli_query($conn, $sql)) {
        $sql_fetch_room_id = "SELECT room_id FROM Displays WHERE display_id = $displayId";
        $result_fetch_room_id = mysqli_query($conn, $sql_fetch_room_id);
        $row_fetch_room_id = mysqli_fetch_assoc($result_fetch_room_id);
        $room_id = $row_fetch_room_id['room_id'];

        header("Location: 1room_devices.php?room_id=$room_id");
        exit();
    } else {
...
// Handle GET request for displaying device form
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['d_id'])) {
    $deviceId = $_GET['d_id'];

    $sql = "SELECT * FROM Displays WHERE display_id = $deviceId";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "No device found with the specified ID.";
    }
}
...
<head>
  ...
  <title>Components / Add Room - NiceAdmin Bootstrap Template</title>
              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="ip_address" name="ip_address" value="<?php echo $ipAddress; ?>" required>
                  </div>
                  <div class="mb-3">
